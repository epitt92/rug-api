...
def validate_referral_code(code: str):
    ...
    if not is_code_format_valid:
        raise InvalidReferralCodeFormat(code, f"Exception: Invalid referral code format, the provided code {code} is not a hexadecimal string of length 6.")

    # TODO: Check if the referral code exists in the database
    is_valid_referral = True

    if not is_valid_referral:
        raise InvalidReferralCode(code, f"Exception: Invalid referral code, the provided code {code} is not valid.")
