...
from src.v1.feeds.constants import (
    ...
    TOP_EVENTS_LIMIT,
    MOST_VIEWED_TOKENS_STALENESS_THRESHOLD,
    MOST_VIEWED_TOKENS_LIMIT,
    MOST_VIEWED_TOKENS_NUM_MINUTES,
    TOP_EVENTS_NUM_MINUTES,
)
from src.v1.feeds.dependencies import (
    process_row,
...
async def post_event_click(eventClick: EventClick):
    ...
    try:
        ...
        logging.error(
            f"An exception occurred whilst parsing the eventClick: {eventClick}. Exception: {e}"
        )
        return JSONResponse(
            status_code=400,
            content={
                "detail": f"An exception occurred whilst parsing the eventClick: {eventClick}. Exception: {e}"
            },
        )
    ...
    return JSONResponse(
        status_code=200,
        content={"detail": f"Event view for {eventHash} from user {userId} recorded."},
    )
...
async def post_token_view(tokenView: TokenView):
    ...
    try:
        ...
        logging.error(
            f"An exception occurred whilst parsing the tokenView: {tokenView}. Exception: {e}"
        )
        return JSONResponse(
            status_code=400,
            content={
                "detail": f"An exception occurred whilst parsing the tokenView: {tokenView}. Exception: {e}"
            },
        )
    ...
    return JSONResponse(
        status_code=200,
        content={
            "detail": f"Token view for token {token_address} on chain {chain} from user {user_id} recorded."
        },
    )
...
async def get_most_viewed_tokens(limit: int = 50):
    ...
    try:
        ...
        logging.error(
            f"Exception: Boto3 exception whilst fetching data from 'feeds' with PK 'mostviewed': {e}"
        )
        raise DatabaseLoadFailureException()
    except Exception as e:
        logging.error(
            f"Exception: Unknown exception whilst fetching data from 'feeds' with PK 'mostviewed': {e}"
        )
        raise DatabaseLoadFailureException()
    ...
    if _most_viewed_tokens:
        ...
        if timestamp:
            ...
            if value and (
                current_time - timestamp < MOST_VIEWED_TOKENS_STALENESS_THRESHOLD
            ):
                        ...
                        if item.get("chain"):
                            try:
                                if isinstance(item.get("chain"), str):
                                    ...
                                    logging.error(
                                        f'Chain was an unexpected type: {type(item.get("chain"))} for item {item}'
                                    )
                                    output[idx]["chain"] = None
                            except Exception as e:
                                logging.error(
                                    f'An exception occurred whilst parsing the chain info {item.get("chain")} for item {item}: {e}'
                                )
                                output[idx]["chain"] = None
                        ...
                        if item.get("score"):
                            try:
                                if isinstance(item.get("score"), str):
                                    ...
                                    logging.error(
                                        f'Score was an unexpected type: {type(item.get("score"))} for item {item}'
                                    )
                                    output[idx]["score"] = None
                            except Exception as e:
                                logging.error(
                                    f'An exception occurred whilst parsing the score info {item.get("score")} for item {item}: {e}'
                                )
                                output[idx]["score"] = None
    ...
    if not found:
        logging.info(
            f"No cached value found for most viewed tokens. Calculating from scratch..."
        )
        output = await get_most_viewed_token_result(
            limit=50, num_minutes=MOST_VIEWED_TOKENS_NUM_MINUTES
        )
        ...
        if not output:
            logging.error(
                f"Exception: No output returned from `get_most_viewed_token_result`."
            )
            raise Exception(
                "No output returned from `get_most_viewed_token_result` query."
            )
        ...
        try:
            ...
            logging.error(
                f"Exception: An exception occurred whilst converting floats to decimals: {e}"
            )
            raise e
        ...
        if len(output) > MOST_VIEWED_TOKENS_LIMIT:
            try:
                ...
                FEEDS_DAO.insert_one(
                    partition_key_value="mostviewed",
                    item={"timestamp": int(time.time()), "value": output},
                )
            except ClientError as e:
            ...
            logging.warning(
                f"The length of the output was {len(output)}, which is less than the limit of {MOST_VIEWED_TOKENS_LIMIT}."
            )
...
async def get_most_viewed_token_result(limit: int = 50, num_minutes: int = 30):
    ...
    def process_row(row):
        try:
            ...
            logging.error(
                f"An exception occurred whilst processing the row: {row}. Exception: {e}"
            )
            return None

        validate_token_address(token_address)

        return {"chain": chain, "token_address": token_address, "count": count}
    ...
    for item in result:
        if item.get("token_address") and item.get("chain"):
            logging.info(
                f'Fetching token details for token {item.get("token_address")} on chain {item.get("chain")}...'
            )
            try:
                token_contract_info = await get_token_details(
                    item.get("chain"), item.get("token_address")
                )
            except Exception as e:
                logging.error(
                    f'An exception occurred whilst fetching token details for token {item.get("token_address")} on chain {item.get("chain")}: {e}'
                )
                continue

            logging.info(
                f'Token name and symbol identified as {token_contract_info.get("name")} ({token_contract_info.get("symbol")})'
            )

            try:
                score_info = await get_score_info(
                    eval(f"ChainEnum.{item.get('chain')}"), item.get("token_address")
                )
            except Exception as e:
                logging.error(
                    f'An exception occurred whilst fetching score info for token {item.get("token_address")} on chain {item.get("chain")}: {e}'
                )
                score_info = None
            ...
            if chain_:
                try:
                    ...
                    logging.error(
                        f'An exception occurred whilst parsing the chain info for token {item.get("token_address")} on chain {item.get("chain")}: {e}'
                    )
                    chain_ = None
            ...
            if score_:
                try:
                    ...
                    logging.error(
                        f'An exception occurred whilst parsing the score info for token {item.get("token_address")} on chain {item.get("chain")}: {e}'
                    )
                    score_ = None
...
async def get_top_events(limit: int = 50):
    ...
    try:
        ...
        logging.error(
            f"Exception: Boto3 exception whilst fetching data from 'feeds' with PK 'topevents': {e}"
        )
        raise DatabaseLoadFailureException()
    except Exception as e:
        logging.error(
            f"Exception: Unknown exception whilst fetching data from 'feeds' with PK 'topevents': {e}"
        )
        raise DatabaseLoadFailureException()
    ...
    if not found:
        logging.info(
            f"No cached value found for top events. Calculating from scratch..."
        )
        output = await get_most_viewed_events_result(
            limit=50, numMinutes=TOP_EVENTS_NUM_MINUTES
        )
        ...
        if len(output) > TOP_EVENTS_LIMIT:
            try:
                ...
                FEEDS_DAO.insert_one(
                    partition_key_value="topevents",
                    item={"timestamp": int(time.time()), "value": output},
                )
            except ClientError as e:
                logging.warning(
                    f"An unknown boto3 exception occurred while writing top events to DAO: {e}"
                )
                raise DatabaseInsertFailureException()
            except Exception as e:
                logging.warning(
                    f"An unknown exception occurred while writing top events to DAO: {e}"
                )
                raise DatabaseInsertFailureException()
...
async def get_most_viewed_events_result(limit: int = 50, numMinutes: int = 30):
    ...
    try:
        ...
        if e.response["Error"]["Code"] == "ThrottlingException":
            message = (
                f"Request was throttled. Consider retrying with exponential backoff."
            )
            logging.error(message)
            raise TimestreamReadException(message=message)
        elif e.response["Error"]["Code"] == "ValidationException":
            message = (
                f"A validation error occurred during call to read from Timestream: {e}"
            )
            logging.error(message)
            raise TimestreamReadException(message=message)
        ...
        message = (
            f"An unknown exception occurred during call to read from Timestream: {e}"
        )
        logging.error(message)
        raise TimestreamReadException(message=message)

    logging.info(
        f'Response fetched successfully! Length of response: {len(response["Rows"])}.'
    )
    ...
    try:
        ...
        df = (
            ...
            .drop(["time", "address", "blockchain", "timestamp"], axis=1)
            .drop_duplicates()
        )
        ...
        for idx, item in enumerate(output):
            try:
                score_info = await get_score_info(
                    eval(f"ChainEnum.{item.get('blockchain')}"), item.get("address")
                )
                score_ = score_info.json() if score_info else None
                ...
                if score_:
                    try:
                        if isinstance(score_, str):
                            ...
                            logging.error(
                                f"Score was an unexpected type: {type(score_)}"
                            )
                            score_ = None
                    except Exception as e:
                        logging.error(
                            f'An exception occurred whilst parsing the score info for token {item.get("address")} on chain {item.get("blockchain")}: {e}'
                        )
                        score_ = None
                ...
                logging.error(
                    f'An exception occurred whilst fetching score info for token {item.get("address")} on chain {item.get("blockchain")}: {e}'
                )
                output[idx]["score"] = None
        ...
        logging.error(
            f"A KeyError exception was thrown during the DataFrame processing step: {e}"
        )
        return []
    except Exception as e:
        logging.error(
            f"An unknown exception was thrown during the DataFrame processing step: {e}"
        )
        return []
...
@router.get(
    "/tokenevents", dependencies=[Depends(decode_token)], include_in_schema=True
)
async def get_token_events(number_of_events: int = 50, chain: ChainEnum = None):
    ...
    where_clause = (
        f"WHERE blockchain = '{chain.value}' AND eventHash IS NOT NULL"
        if chain
        else "WHERE eventHash IS NOT NULL"
    )
    ...
    # Handle the data as a pandas dataframe to pivot the data
    df = pd.DataFrame(processed_rows).drop(
        ["time", "address", "blockchain", "timestamp"], axis=1
    )
...
async def get_token_details(chain: ChainEnum, token_address: str):
    ...
    try:
        ...
        logging.warning(
            f"An exception occurred whilst fetching token details for token {token_address} on chain {chain}: {e}"
        )
        raise e
