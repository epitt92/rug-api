...
async def fetch_raw_code(
    ...
    if not prefix:
        logging.error(
            f"The environment variable '{_chain.upper()}_BLOCK_EXPLORER_URL' was not found."
        )
        raise Exception(
            f"The environment variable '{_chain.upper()}_BLOCK_EXPLORER_URL' was not found."
        )
    ...
    if not api_key:
        logging.error(
            f"The environment variable '{_chain.upper()}_BLOCK_EXPLORER_API_KEY' was not found."
        )
        raise Exception(
            f"The environment variable '{_chain.upper()}_BLOCK_EXPLORER_API_KEY' was not found."
        )
    ...
    try:
        ...
        logging.error(
            f"Exception: During the call to GET getsourcecode from the Block Explorer for token {token_address} on chain {chain}: {e}"
        )
    ...
    try:
        ...
        logging.error(
            f"Exception: An error occurred whilst attempting to handle the `data` object: {data}. Exception: {e}"
        )
        return None
...
async def parse_raw_code(source: str) -> dict:
    ...
    if source.startswith("{"):
        ...
        try:
            ...
            logging.error(
                f"Exception: While loading `clean_source` into a JSON object: {clean_source}. Exception: {e}"
            )
            raise Exception(
                f"Exception: While loading `clean_source` into a JSON object: {clean_source}. Exception: {e}"
            )
...
async def get_source_code_map(
    chain: ChainEnum, token_address: str = Depends(validate_token_address)
):
    logging.debug(
        f"Fetching source code map for {token_address} on chain {chain}, via map..."
    )
    ...
    try:
        ...
        logging.error(
            f"Exception: During nested call to fetch and parse source code for {token_address} on chain {chain}: {e}"
        )
        raise Exception(
            f"Exception: During nested call to fetch and parse source code for {token_address} on chain {chain}: {e}"
        )
    ...
    if isinstance(source, str):
        if len(source) == 0:
            ...
            return {
                "Flattened Source Code": "No source code available for this contract."
            }
        return {"Flattened Source Code": source}
    elif isinstance(source, dict):
        # Pre-process dictionary keys and return
        new_keys = [
            file.split("/")[-1] for file in source.keys() if file.endswith(".sol")
        ]

        if len(new_keys) == 0 and len(source.keys()) == 1:
            return {
                "Flattened Source Code": source.get(list(source.keys())[0]).get(
                    "content"
                )
            }
        elif len(new_keys) == 0:
            logging.error(
                f"Exception: The new keys list was empty for token {token_address} on chain {chain}."
            )
            raise Exception(
                f"Failed to fetch raw source code file, or no source code available for this contract."
            )

        key_map = dict(zip(source.keys(), new_keys))
        return {
            key_map.get(key): source.get(key).get("content") for key in source.keys()
        }
...
async def get_source_code(
    ...
    if _chain != "ethereum":
        logging.error(
            f"Exception: The chain {chain} is not supported by the `get_source_code` endpoint."
        )
        raise UnsupportedChainException(chain=chain)

    logging.debug(
        f"Fetching source code for {token_address} on chain {chain} from the database..."
    )
    ...
    try:
        ...
        logging.error(
            f"Exception: Boto3 exception whilst fetching data from 'sourcecode' with PK: {pk}"
        )
        raise DatabaseLoadFailureException()
    except Exception as e:
        logging.error(
            f"Exception: Unknown exception whilst fetching data from 'sourcecode' with PK: {pk}"
        )
    ...
    if not _source_code_map:
        logging.debug(
            f"No source code found for {token_address} on chain {chain}. Fetching from API..."
        )
        ...
        if _source_code_map is not None:
            try:
                logging.info(
                    f"Writing source code map for {token_address} on chain {chain} to DAO..."
                )
                SOURCE_CODE_DAO.insert_one(
                    partition_key_value=pk,
                    item={
                        "timestamp": int(time.time()),
                        "sourcecode": _source_code_map,
                    },
                )
            except ClientError as e:
                if e.response["Error"]["Code"] == "ConditionalCheckFailedException":
                    logging.error(
                        f"Exception: Source code map for {token_address} on chain {chain} already exists in DAO."
                    )
                    logging.error(
                        f"Exception: We do not allow duplicate writes for source code, since it is immutable."
                    )
                else:
                    logging.error(
                        f"Exception: An unknown boto3 exception occurred while writing source code map for {token_address} on chain {chain} to DAO: {e}"
                    )

                raise DatabaseInsertFailureException()
            except Exception as e:
                logging.warning(
                    f"Exception: An unknown exception occurred while writing source code map for {token_address} on chain {chain} to DAO: {e}"
                )
                raise DatabaseInsertFailureException()
        else:
            raise Exception(
                f"No source code found for {token_address} on chain {chain.value}"
            )
    else:
    ...
    try:
        ...
        logging.error(
            f"Exception: A ValidationError was raised for SourceCodeFile: {e.json()}"
        )
        raise OutputValidationError()
    except Exception as e:
        logging.error(
            f"Exception: An uncaught exception occurred was raised for SourceCodeFile: {e}"
        )
        raise OutputValidationError()
    ...
    try:
        ...
        logging.error(
            f"Exception: A ValidationError was raised for SourceCodeResponse: {e.json()}"
        )
        raise OutputValidationError()
    except Exception as e:
        logging.error(
            f"Exception: An uncaught exception occurred was raised for SourceCodeResponse: {e}"
        )
        raise OutputValidationError()
