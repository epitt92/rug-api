...
from src.v1.shared.exceptions import (
    ...
    GoPlusAccessKeyLoadError,
    GoPlusAccessKeyRefreshError,
)
...
def get_rpc_provider(chain: ChainEnum):
    ...
    if _chain == "ethereum":
        ...
        raise UnsupportedChainException(
            f"Invalid Chain on call to `get_chain`: {_chain}"
        )
...
async def get_token_contract_details(chain: ChainEnum, token_address: str) -> dict:
    ...
    try:
        ...
        logging.warning(
            f"An exception occurred whilst trying to create an ERC20 object for token {token_address} on chain {chain}: {e}"
        )
    ...
    try:
        ...
        logging.warning(
            f"An exception occurred whilst trying to fetch token details for token {token_address} on chain {chain}: {e}"
        )
        raise e
...
def fetch_access_token():
    ...
    try:
        token_data = auth.Auth(
            key=os.getenv("GO_PLUS_APP_KEY"), secret=os.getenv("GO_PLUS_APP_SECRET")
        ).get_access_token()
    except Exception as e:
        logging.error(
            f"Exception: An exception occurred whilst attempting to fetch a new access token from GoPlus."
        )
    ...
    try:
        ...
        logging.warning(
            f"Exception: An exception occurred whilst attempting to parse the access token response from GoPlus."
        )
        raise GoPlusAccessKeyRefreshError()
    ...
    try:
        ...
        logging.warning(
            f"Exception: The access token file could not be found whilst trying to save a new access token from GoPlus."
        )
        raise GoPlusAccessKeyRefreshError()
    except Exception as e:
        logging.warning(
            f"Exception: An exception occurred whilst attempting to save a new access token from GoPlus."
        )
        raise GoPlusAccessKeyRefreshError()
