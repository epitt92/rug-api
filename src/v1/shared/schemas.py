...
class TokenBase(BaseModel):
    ...
    def validate_token_address(cls, value):
        if not value.startswith("0x"):
            raise ValueError(
                'Field "tokenAddress" must be a valid Ethereum address beginning with "0x".'
            )
        if len(value) != 42:
            raise ValueError(
                'Field "tokenAddress" must be a valid Ethereum address with length 42.'
            )
        return value
